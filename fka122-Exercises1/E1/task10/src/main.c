...
int main()
{
    ...
    double **energies_time = create_2D_array(time_saves, N);
    ...
    double *normal_mode_energy_time = (double*)calloc(time_saves, sizeof(double));
    double **cumulative_integration_normal_energy = create_2D_array(N, time_saves);
    
    for(unsigned int k = 0; k < N; k++){
        for(unsigned int j = 0; j < time_saves; j++){
            normal_mode_energy_time[j] = energies_time[j][k];
        }
        
        cumulative_integration(cumulative_integration_normal_energy[k],
                               normal_mode_energy_time,
                               timestep*1000,
                               time_saves);
    }
    ...
    /*
    FILE *file_ptr;
    file_ptr = fopen("energies-task10-alpha1.csv", "w");
    if (file_ptr == NULL) {
        perror("Error opening file");
        return 1;
    }
    for(unsigned int i = 0; i < time_saves; i++){
